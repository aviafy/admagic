...
class ContentAnalyzerService {
    constructor(openaiModel, gemini, preferredProvider) {
        ...
        this.logger.log(`🔧 [ContentAnalyzer] Initialized with preferredProvider: ${preferredProvider}, Gemini available: ${gemini !== null}`);
    }
    async analyze(content, contentType) {
        this.logger.log(`🔍 [ContentAnalyzer.analyze] Starting analysis with preferredProvider: ${this.preferredProvider}`);
        this.logger.log(`📝 [ContentAnalyzer.analyze] Gemini available: ${this.gemini !== null}`);
        ...
        try {
            ...
            if (this.preferredProvider === constants_1.AIProvider.GEMINI && this.gemini) {
                this.logger.log(`🟢 [ContentAnalyzer.analyze] Preferred provider is GEMINI, attempting Gemini analysis...`);
                try {
                    analysisResult = await this.analyzeWithGemini(prompt);
                    usedProvider = constants_1.AIProvider.GEMINI;
                    this.logger.log("✅ [ContentAnalyzer.analyze] Successfully used Gemini for analysis");
                }
                catch (error) {
                    this.logger.error(`❌ [ContentAnalyzer.analyze] Gemini failed, falling back to OpenAI. Error: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error.stack : undefined);
                    analysisResult = await this.analyzeWithOpenAI(prompt);
                    usedProvider = constants_1.AIProvider.OPENAI;
                    this.logger.log("✅ [ContentAnalyzer.analyze] Fallback to OpenAI successful");
                }
            }
            else {
                this.logger.log(`🔵 [ContentAnalyzer.analyze] Preferred provider is OPENAI or Gemini unavailable, using OpenAI...`);
                try {
                    analysisResult = await this.analyzeWithOpenAI(prompt);
                    usedProvider = constants_1.AIProvider.OPENAI;
                    this.logger.log("✅ [ContentAnalyzer.analyze] Successfully used OpenAI for analysis");
                }
                catch (error) {
                    if (this.gemini) {
                        this.logger.warn("⚠️ [ContentAnalyzer.analyze] OpenAI failed, falling back to Gemini", error);
                        analysisResult = await this.analyzeWithGemini(prompt);
                        usedProvider = constants_1.AIProvider.GEMINI;
                        this.logger.log("✅ [ContentAnalyzer.analyze] Fallback to Gemini successful");
                    }
                    else {
                        throw error;
                    }
